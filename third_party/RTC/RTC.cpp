...
Callback<void()> RTC::attachCB[6];
Callback<void()> RTC::alarmCB;
...
void RTC::attach(void (*function)(void), TimeUnit interval)
{
    ...
    attachCB[interval] = callback(function);
    _attach(interval);
}
...
{
    //Set the function pointer
    attachCB[interval] = callback(object, member);
    _attach(interval);
}
...
void RTC::detach(TimeUnit interval)
{
    ...
    attachCB[interval] = Callback<void()>();
}
...
{
    //Set the function pointer
    alarmCB = callback(object, member);
    _alarm(alarmTime);
}
